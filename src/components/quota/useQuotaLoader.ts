...
import { useQuotaCacheStore } from '@/stores/useQuotaCacheStore';
...
export function useQuotaLoader<TState, TData>(config: QuotaConfig<TState, TData>) {
  ...
  const quota = useQuotaCacheStore(config.storeSelector);
  const setQuota = useQuotaCacheStore((state) => state[config.storeSetter]) as QuotaSetter<
